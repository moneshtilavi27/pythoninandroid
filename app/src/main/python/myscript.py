...
#      img1 = cv2.imread(''+str(img1)+'',0)
#      print(img1)
     return matching_function(image,path)


def matching_function(image,path): #can add or remove desti
  image=image
  comm=path
  ...
  for folder in folders:
    ...
    if(os.path.exists(str(path)+'/'+str(folder)+'/'+str("newFrames"))):
        if(os.path.exists(str(path)+'/'+str(folder)+'/'+str("newFrames")+'/'+str("grayscale"))):
            cpp=''+str(path)+'/'+str(folder)+'/'+str("newFrames")+'/'+str("grayscale")+'/'
            comparewith = [ name for name in os.listdir(path) if not os.path.isdir(os.path.join(path, name)) ]
            print(cpp)

            print(comparewith)
            for cp in comparewith:
              print("image 2 ")
              img1 = cv2.imread(''+str(image)+'',0) # specific image to compare
